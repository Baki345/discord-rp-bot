...
import { buyFromShop, ServiceError } from "@discord-rp/core";
...
export async function executeAcheter(interaction: ChatInputCommandInteraction) {
  ...
  try {
    const result = await buyFromShop(actor, { guildId: actor.guildId, shopId, characterId: character.id, itemId, quantity });
    await interaction.reply({ content: `✅ Acheté pour **${(result.totalCents / 100).toFixed(2)} $**.`, ephemeral: true });
  } catch (e) {
    if (e instanceof ServiceError && e.code === "INSUFFICIENT_CASH") {
      await interaction.reply({ content: "Tu n'as pas assez de liquide.", ephemeral: true });
      return;
    }
